...
- **Usability Auditor (UX/юзабилити)**: оценивает UX диалогов/кнопок/состояний бота, находит фрикцию и неоднозначности, предлагает улучшения (тексты, навигация, ошибки, таймауты, “устаревшие” сообщения). Код не правит без явного разрешения.
...
Примечание по skills: **Architect** под задачу явно выбирает подходящие skills (например `review-diff`, `security-check`, `quality-gate`, `docs-update`) и запускает их по необходимости.
